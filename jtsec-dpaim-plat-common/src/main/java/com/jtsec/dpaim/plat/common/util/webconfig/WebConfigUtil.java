package com.jtsec.dpaim.plat.common.util.webconfig;
...
public class WebConfigUtil {
	...
	private static File getWebConfFilePath(){
		...
		File file = new File(FileUtil.getFilePath ("controller-config.conf"));
	...
	public static void bakWebConfig(){
		...
		File file = new File(FileUtil.getFilePath ("controller-config.conf"));
		//备份文件以便用于回复默认值
		File destFile = new File(file.getParent (), "controller-config.conf-bak");
		try {
...
//		}
	...
	public static void reset(){
		...
		File fileSrc = new File(filePath, "controller-config.conf-bak");
		File file = new File(filePath, "controller-config.conf");
		try {
